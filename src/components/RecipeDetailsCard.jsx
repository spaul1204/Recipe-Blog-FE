...
const RecipeDetailsCard = () => {
  ...
  const addFavoriteHandler = async () => {
    ...
    try {
      ...
      if (response.data && response.data.success) {
        ...
        console.log(
          "Could not add recipe to favorites (API success, but operation failed):",
          response.data.message ||
            "Backend indicated operation was not successful."
        );
        setToastMessage(response.data.message || "Could not save recipe.");
        setShowToast(true);
        setTimeout(() => {
          setShowToast(false);
          setToastMessage("");
      ...
      console.error(
        "Error adding recipe to favorites (API call failed):",
        error
      );
      setToastMessage("Error saving recipe. Please try again.");
      setShowToast(true);
      setTimeout(() => {
        setShowToast(false);
        setToastMessage("");
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          console.error(
            "Unauthorized: Backend rejected the request. Check token or credentials for this GET request."
          );
        }
      }
    }
  ...
  return (
    <>
      {/* Toast Notification Container - Placed at the top for visibility control within this component structure */}
      {showToast && (
        <div className="toast toast-bottom toast-center sm:toast-end z-50 p-4">
          <div
            className={`alert ${
              toastMessage.startsWith("Error") ||
              toastMessage.startsWith("Could not")
                ? "alert-error"
                : "alert-success"
            }`}
          >
        ...
        {recipe?.recipeTag && (
          <div className="w-2/3 flex flex-wrap gap-2 mb-4">
            {recipe?.recipeTag.map((tag, idx) => (
              <span
                key={idx}
                className="badge badge-sm badge-primary badge-outline text-xs text-base-content py-1 px-2"
              >
                {tag.trim()}
              </span>
            ))}
